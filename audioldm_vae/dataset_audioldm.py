...
class PrecomputedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # print("==", dpath)
            data = torch.load(dpath, weights_only=True, map_location='cpu')
        except:
        ...
        return latent, embeds, data["masks"][index], data["captions"][index], data["meta_info"]
        # return latent.cpu().to(self.dtype), embeds, data["masks"][index], data["captions"][index], data["meta_info"]
...
#         latent, embeds = data["latents"], data["embeds"]
#         return latent[0], embeds[0], data["masks"][0]